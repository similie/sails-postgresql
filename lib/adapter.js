...
function getPool(connectionConfig, connectionName) {
  // console.log('CONNECTION CONFIG', connectionConfig);
  if (connectionPool[connectionName]) {
    return connectionPool[connectionName];
  }
  connectionPool[connectionName] = new Pool({
    ...
    max: connectionConfig.poolSize || 40,
    idleTimeoutMillis: connectionConfig.idleTimeoutMillis || 30000,
    connectionTimeoutMillis: connectionConfig.connectionTimeoutMillis || 10000,
  });
  return connectionPool[connectionName];
}
  ...
  var adapter = {
    ...
    terminatePool: function(conn, cb) {
      if (conn && connectionPool[conn]) {
        return connectionPool[conn].end(cb);
      } else if (conn) {
        return cb();
      }
      for (const key in connectionPool) {
        connectionPool[key].end();
      }
      cb();
    },
  ...
  function spawnConnection(connectionName, logic, cb) {
    ...
    const pool = getPool(connectionConfig, connectionName);
    pool.connect().then((client) => {
      after(null, client, async () => {
        await client.release();
      });
    }).catch((err) => {
      after(err, null, _.noop);
    });
